...
- [`business/locks-and-cache.example.ts`](./business/locks-and-cache.example.ts)
  Shows both the current scaffold wrappers and a more standardized `cache-manager + Keyv + Redlock` approach.
...
- [`business/async-task/task-definitions.example.ts`](./business/async-task/task-definitions.example.ts)
- [`business/async-task/enqueue.example.ts`](./business/async-task/enqueue.example.ts)
- [`business/async-task/worker.example.ts`](./business/async-task/worker.example.ts)
- [`business/async-task/scheduler.example.ts`](./business/async-task/scheduler.example.ts)
  A full async task example covering definition, enqueue, worker consumption, and recurring scheduling.
